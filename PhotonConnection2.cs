...
public class PhotonConnection2 : MonoBehaviourPunCallbacks {
    ...
    public override void OnJoinedRoom()
    {
        ...
        GameObject car = PhotonNetwork.Instantiate(nombrePrefabEnResources, new Vector3(32f + (2* PhotonNetwork.PlayerList.Length), 5f, 18f), Quaternion.identity, 0);
        //Habilitamos todos sus componentes
        foreach(MonoBehaviour m in car.GetComponents<MonoBehaviour>())
        {
            if (!m.enabled)
                m.enabled = true;
                 if (!car.GetComponentInChildren<Camera>().enabled)
